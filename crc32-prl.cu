...
__global__
void crc32kernel(uint8_t* data, uint32_t* crc, int length, uint32_t *d_table) {
    /// YOUR CODE
    ...
    if(id < 2) { 
        ...
        crc[id] = d_table[(in_crc ^ value) & 0xFFL];
...
__global__
void xorkernel(uint8_t* data, uint32_t* crc, int length, uint32_t * d_table) {
    /// YOUR CODE
    int id = blockIdx.x * blockDim.x + threadIdx.x;
    if(id == 0) {
        printf("Ox%x - Ox%x\n",crc[id], crc[id+1]);
...
int main() {
    ...
    uint8_t* ddata;
    uint32_t *d_table;
    cudaMalloc(&dcrc, 2*sizeof(uint32_t));
    cudaMalloc(&ddata, length*sizeof(uint8_t));
    cudaMalloc(&d_table, 256 * sizeof(uint32_t));
    ...
    cudaMemcpy(d_table, table, 256 * sizeof(uint32_t), cudaMemcpyHostToDevice);
    ...
    crc32kernel<<< num_blocks, block_size >>>(ddata, dcrc, length, d_table);
    printf("1\n");
    xorkernel<<< 1, 1 >>>(ddata, dcrc, length, d_table);
    printf("2\n");
