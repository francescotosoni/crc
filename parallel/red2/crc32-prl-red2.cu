...
void xorkernel(uint32_t* d_input) {
    __shared__ uint32_t partialSum[DIM]; 

	partialSum[threadIdx.x] = d_input[blockIdx.x * (blockDim.x * 2) + threadIdx.x];
	partialSum[threadIdx.x + blockDim.x] = d_input[blockIdx.x * (blockDim.x * 2) + threadIdx.x + blockDim.x];
    ...
    if(t == 0){
        d_input[blockIdx.x] = partialSum[0];
    }
}
...
int main() {
    ...
    for(int i = 0; i < N_iter; i++){
        xorkernel<<<ceil((float)length/tile_temp), (DIM/2)>>>(tmp);
        tile_temp *= DIM;
    }
    TM_device.stop();
